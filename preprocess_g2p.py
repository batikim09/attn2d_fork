...
def main_trg(params, train_order, val_order, test_order, vocab=None):
    ...
    with h5py.File('data/%s/%s.h5' % (params.data_dir, params.trg), "w") as f:
...
def main_src(params):
    ...
    # create output h5 file
    with h5py.File('data/%s/%s.h5' % (params.data_dir, params.src), "w") as f:
    ...
    print('wrote h5file for the source (X)')
    pd({'itow': itow, 'params': params},
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='en_us')
    parser.add_argument('--src', type=str, default='src')
    parser.add_argument('--trg', type=str, default='tgt')
